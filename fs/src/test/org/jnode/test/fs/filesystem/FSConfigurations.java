...
public class FSConfigurations implements Iterable<FSTestConfig> {
    ...
    public FSConfigurations() {
        ...
        // FSAccessMode.BOTH, new Ext2FileSystemFormatter(BlockSize._1Kb),
        // diskFileName, "1M"));
        //
        // configs.addAll(createFileConfigs(OsType.OTHER_OS, FSType.EXT2,
        // FSAccessMode.BOTH, new Ext2FileSystemFormatter(BlockSize._4Kb),
        // diskFileName, "1M"));

        // configs.addAll(createFileConfigs(OsType.OTHER_OS, FSType.NTFS,
        // FSAccessMode.BOTH, "", DO_FORMAT, diskFileName, "1M"));

        configs.addAll(createFileConfigs(OsType.OTHER_OS, FSType.HFS_PLUS, FSAccessMode.BOTH,
                new HfsPlusFileSystemFormatter(new HFSPlusParams()), diskFileName, "64M"));

        // configs.addAll(createFileConfigs(OsType.OTHER_OS, FSType.FAT,
        // FSAccessMode.BOTH, new FatFileSystemFormatter(FatType.FAT12),
        // diskFileName, "1M"));
        //
        // configs.addAll(createFileConfigs(OsType.OTHER_OS, FSType.FAT,
        // FSAccessMode.BOTH, new FatFileSystemFormatter(FatType.FAT16),
        // diskFileName, "1M"));
        //
        // configs.addAll(createFileConfigs(OsType.OTHER_OS, FSType.FAT,
        // FSAccessMode.BOTH, new FatFileSystemFormatter(FatType.FAT32),
        // diskFileName, "1M"));
        //
         ...
         * FSAccessMode.BOTH, "1", DO_FORMAT)); //<workDevice name="hdb5" />
         * 
         * configs.addAll(createConfigs(OsType.JNODE_OS, FSType.EXT2,
         * FSAccessMode.BOTH, null, DO_NOT_FORMAT)); //<workRamdisk size="1M" />
         */
    }

    private List<FSTestConfig> createFileConfigs(OsType osType, FSType fsType,
            FSAccessMode accessMode, Formatter<? extends FileSystem<?>> formatter, String fileName,
            String fileSize) {
        FileParam fp = new FileParam(fileName, fileSize);
        return createConfigs(osType, fsType, accessMode, formatter, fp);
    }

    private List<FSTestConfig> createConfigs(OsType osType, FSType fsType, FSAccessMode accessMode,
            Formatter<? extends FileSystem<?>> formatter, DeviceParam device) {
